...
RESULTS_DIR = "results"
PRODUCT = "FubuTransportation"
COPYRIGHT = "Copyright 2008-#{Time.now.year} Chad Myers, Jeremy D. Miller, Joshua Flanagan, et al. All rights reserved."
COMMON_ASSEMBLY_INFO = 'src/CommonAssemblyInfo.cs';
BUILD_DIR = File.expand_path("build")
ARTIFACTS = File.expand_path("artifacts")

@teamcity_build_id = "btxx"
...
task :default => %w{compile unit_test}

desc "Unit and Integration Tests"
task :full => %w{default integration_test}

desc "Target used for the CI server"
task :ci => %w{update_all_dependencies default integration_test history package}

desc "Target used for CI on Mono"
task :mono_ci => %w{update_all_dependencies compile mono_unit_test integration_test}
...
task :test => %w{unit_test}

desc "Runs unit tests"
task :unit_test => :compile do
  runner = NUnitRunner.new :compilemode => COMPILE_TARGET, :source => 'src', :platform => 'x86'
  runner.executeTests %w{FubuTransportation.Tests}
end
